...
export default async function CounterPage({ searchParams }: CounterPageProps) {
  ...
  const baseUrl = `/counter?${baseParams.toString()}`;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold">Counter Trackers</h1>
            <p className="text-muted-foreground text-sm">
              Count occurrences, repetitions, and quantities
            </p>
          </div>
        </div>
      ...
      <TrackerFilters
        ...
        defaultType={TrackerType.COUNTER}
      ...
      <div className="space-y-4">
        {trackers.length > 0 ? (
          ...
          <div className="bg-background border-border rounded-lg border p-6 text-center">
            <p className="text-foreground/70">No counter trackers found</p>
            <Link href="/trackers/new?type=COUNTER" className="mt-2 inline-block">
              <Button variant="outline" size="sm">
                Create Your First Counter Tracker
              </Button>
            </Link>
          </div>
        )}
